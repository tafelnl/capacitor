...
Unless `insetsHandling` is set to `disable`, the plugin also enables edge-to-edge mode on the activity window (`EdgeToEdge.enable`), so the window state matches what the insets handling assumes on every Android version, not only on Android 15+ where the system enforces it. Set `insetsHandling` to `disable` if your app manages the window and insets itself.
